...
三个 `nn.Linear` 就是三个矩阵，把每个字的向量投影到 Q、K、V 三个不同的"空间"里。为什么不用偏置（`bias=False`）？这是 Transformer 的惯例，实验表明去掉偏置效果差不多，还能少点参数。
...
每个字的 n_embd 维向量，分别经过三个不同的线性变换，得到三种不同的"视角"。
...
上三角为 0 的地方被填成 `-inf`（负无穷），经过 softmax 后这些位置的权重变成 0 —— 等于"看不见"后面的字。
