...
4. **匹配与关注**：每个字用自己的 Q 去和所有字的 K **逐一比较**，算出一个"关注度"分数
   - 怎么比较？用**点积**（两个向量对应位置相乘再求和）。Q 和 K 越"朝同一个方向"，点积越大，关注度越高
   - 举个例子："兵"的 Q 向量是 [0.8, 0.3, -0.1, ...]，"曹操"的 K 向量是 [0.7, 0.4, 0.0, ...]，点积 = 0.8×0.7 + 0.3×0.4 + ... = 一个较大的正数 → 说明很匹配，"兵"应该多关注"曹操"
   - 而"兵"的 Q 和"却说"的 K 点积很小 → 不太匹配，少关注
   - 算完所有点积后，再经过 **softmax** 把分数转成概率（加起来等于 1），这样每个字就得到了一组"关注度权重"：比如对"曹操"关注 0.4，对"引"关注 0.3，对"却说"关注 0.05……
