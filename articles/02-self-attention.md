...
### Token Embedding（词嵌入）：从"编号"到"画像"

在 Bigram 里，每个字就是一个编号——"曹"是 1038，"操"是 2893。一个数字能描述一个字的含义吗？显然不能。1038 和 1039 只是编号相邻，并不意味着这两个字有任何关系。

所以 Self-Attention 做的第一件事就是：**别再用一个编号来代表一个字了，改用一组数字（向量）来描述它。**

这就像警察找人——光靠身份证号没法判断两个人像不像。你需要的是一份"人物画像"：身高、体重、年龄、肤色……用这些特征来描述一个人，才能做比较、做分析。

Embedding 层做的就是这件事：把每个字的编号，变成一组 64 个数字的"画像向量"。这 64 个数字具体代表什么？我们不指定——让模型自己在训练过程中学出来。训练完成后，含义相近的字，它们的 64 个数字会比较接近。

理解了"为什么要 64 维"之后，我们来对比一下两个模型的完整数据流：
