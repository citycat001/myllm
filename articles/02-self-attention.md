...
在 Bigram 里，每个字就是一个编号——"曹"是 1038，"操"是 2893。这个编号就像你的身份证号：它只是一个**标识符**，用来区分"你是谁"，仅此而已。

但一个编号有个致命问题：**它无法表达字与字之间的关系。**

举个例子：
- "曹" 编号 1038，"操" 编号 2893，"刘" 编号 987，"备" 编号 3421
- 从编号上看，1038 和 987 差了 51，1038 和 2893 差了 1855
- 但我们知道，"曹"和"刘"都是**姓氏**，应该更"像"；"曹"和"操"经常**一起出现**，关系也很紧密
- 光靠一个编号，模型完全看不出这些关系——它只看到四个毫无规律的数字

这就像用身份证号来判断两个人是否相似——110101 和 110102 只是号码挨着，不代表两个人长得像、性格像、或者有任何关系。

**解决办法：用一组数字来描述每个字，而不是一个数字。**

想象一下，我们给每个字建一份"人物档案"，用多个维度来描述它的特征：
...
这样一来：
- "曹"和"刘"在"是否常做姓氏"这个维度上都很高，模型就能发现它们是同类
- "曹"和"操"在"是否常在人名中"这个维度上都很高，模型就能发现它们常一起出现
- "的"在所有特征维度上都接近 0，模型就知道它是个功能词，和人名无关

**这就是 Embedding 做的事：把一个编号（1 维）升维成一个向量（64 维），让每个字从一个"身份证号"变成一份"多维画像"。**

这里有两个常见问题：

**问：这 64 个维度具体代表什么？** 答：我们不指定！上面表格里的"是否常做姓氏"只是举例帮助理解。实际上，我们只告诉模型"你有 64 个格子可以用"，具体每个格子存什么含义，模型在训练过程中自己学。训练完成后，含义相近的字，它们的 64 个数字自然会比较接近。

**问：为什么是 64 维，不是 10 维或 1000 维？** 答：这是一个需要调的超参数。维度太小（比如 10），描述能力不够，很多字会"挤"在一起分不开；维度太大（比如 1000），参数量剧增，训练变慢还容易过拟合。64 是我们为这个小模型选的平衡点——足以描述几千个汉字的特征，又不至于太浪费。

**问：升维之后对注意力机制有什么好处？** 答：这是最关键的一点。注意力机制的核心操作是**比较两个字有多相关**（后面会详细讲）。要比较，就需要有"特征"可以比——你没法比较两个身份证号谁和谁更"相关"，但你可以比较两份画像的相似度。64 维的向量就给了注意力机制一个可以做数学运算的空间：可以算相似度、可以提取信息、可以融合上下文。**没有这个升维，注意力机制根本无从下手。**
