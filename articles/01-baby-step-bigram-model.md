...
在喂给模型之前，我们要先把文字变成数字，因为计算机不懂得自然语言文字，只懂得数字。最简单的方式就是**字符级分词**：每个字符（包括汉字、标点、空格）分配一个唯一的整数编号。
...
在我们的场景中，这本"字典"有 4742 页（每个字一页），每页写着 4742 个数字（对下一个字的打分）。所以整张表就是 4742 × 4742 = **22,486,564**（约 2250 万）个数字，这就是模型需要学习的全部内容。一开始这张表里是被自动赋予的随机数。

它和普通的 Excel 表格的区别是：`nn.Embedding` 能**参与训练**。在训练过程中，PyTorch 会自动帮我们调整表里的数字，让预测越来越准。如果只用普通数组，我们就得自己手写一堆数学公式来更新数值，麻烦得多。后续训练的目的就是把这张表里的数字更新成最符合三国演义的行文的新数字，后续预测新字的时候会像三国演义写的一样。
...
- **C** (Classes) = 对 4742 个候选字的打分数组，好比每道题附带一张"各选项得分表"

**注意 C 的含义**：C 不是"这个字本身有什么特征"，而是一个长度 4742 的数组，里面记着"下一个字是'操'得几分、是'军'得几分、是'的'得几分……"。就像一道选择题后面的答题卡——上面 4742 个选项各有一个分数，不是在描述题目本身。

输入 `idx` 形状是 `(B, T)` —— 64 组字串，每组 8 个字。经过 Embedding 查表后得到 `(B, T, C)` —— 每个位置都附上了一份"预测下一个字"的打分数组。

**训练和生成对这个输出的用法不同：**

- **训练时**：8 个位置**全部用到**。"却"预测"说"、"说"预测"曹"、"曹"预测"操"……一条样本产生 8 个学习机会，全部参与 loss 计算。这就是为什么后面展平时是 `64 × 8 = 512` 个样本。
- **生成时**：只取**最后一个位置**的预测（`logits[:, -1, :]`）。比如输入"却说曹操"，我们只关心"操"后面接什么字，前面几个位置的预测结果直接丢掉。

### 同一个 forward，训练和生成都用它

你可能注意到了：`forward` 函数只写了一套逻辑，但训练和生成都调用它。区别在于：

| | B（批次大小） | T（序列长度） | targets |
|---|---|---|---|
| **训练** | 64（一批 64 条样本） | 8（固定长度） | 传入正确答案，算 loss |
| **生成** | 1（只有一条序列） | 不断增长（每生成一个字 +1） | 不传，loss=None |

为什么生成时 B=1 也要保留 batch 维度？因为 PyTorch 规定所有输入必须带 batch 维度。就像餐厅后厨——不管是 64 桌同时点菜还是只有 1 桌，用的都是同一套出餐流程。生成时只是"这批订单刚好只有 1 份"而已。

### 为什么用 cross_entropy（交叉熵）？

训练时，T 个位置的**每个字都在预测它后面的下一个字**，然后和正确答案逐个对比看差距：

```
x = [却, 说, 曹, 操, 引, 兵, 追, 赶]    ← 输入（8 个字）
y = [说, 曹, 操, 引, 兵, 追, 赶, 关]    ← 正确答案（每个字的下一个字）

cross_entropy 做的事：
  "却"的预测打分 vs 正确答案"说" → 算一个 loss
  "说"的预测打分 vs 正确答案"曹" → 算一个 loss
  "曹"的预测打分 vs 正确答案"操" → 算一个 loss
  ……8 个位置各算一个 loss，最后取平均
```

每个位置都是一道 **4742 选 1 的选择题**——模型给 4742 个候选字各打了一个分，cross_entropy 看正确答案拿到了多少分：
- 正确答案得分高（比如 60%）→ loss 小（猜得不错）
- 正确答案得分低（比如 1%）→ loss 大（猜得很差）

所以 loss 就像考试扣分——猜得越离谱，扣分越狠。一条样本 8 个位置、一批 64 条样本，总共 **512 个选择题**，cross_entropy 算出所有题的平均扣分。模型通过不断减少这个平均扣分来学会给正确答案更高的概率。

### 为什么需要 view/reshape？

这是一个纯粹的"格式要求"问题。

PyTorch 的 `F.cross_entropy` 要求输入摆成 `(样本数, 类别数)` 的二维表格。但我们的预测结果是三维的 `(64, 8, 4742)` —— 64 组、每组 8 个位置、每个位置 4742 个候选字的打分。

解决办法：把 512 个位置**展平**成 512 个独立的选择题，变成 `(512, 4742)` 的二维表格。正确答案同理，从 `(64, 8)` 展平成 `(512,)`。这样 cross_entropy 就能一次性给 512 道选择题打分了。
...
注意这里的 `idx` 形状是 `(1, T)`——B=1（只有一条序列），T 每生成一个字就 +1。比如输入"却说曹操"时 T=4，生成第一个字后 T=5，第二个字后 T=6……序列不断变长，直到写够指定数量。

这和训练时不同：训练时 `(64, 8)` 是固定形状，生成时 `(1, T)` 是动态增长的。但 `forward` 不在乎——它只管拿到 `(B, T)` 就查表返回 `(B, T, C)`，不管 B 是 64 还是 1，T 是 8 还是 100。
