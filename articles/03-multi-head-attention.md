...
你可能会想：既然一个头不够，把维度从 64 加到 256 不就行了？

答案是：**不行，因为瓶颈不在维度大小，而在 softmax 的结构性限制。**

回忆上一篇：注意力的核心是 Q 和 K 做点积后过 **softmax**，产生一组注意力权重。这组权重是一个概率分布——所有位置的权重加起来等于 1。

看看 "追赶" 这个位置往回看时会发生什么：

```
单头 softmax（只能产生一个概率分布）：
  曹:0.05  操:0.30  引:0.10  兵:0.05  追:—  赶:—  关:0.40  公:0.10
                                                               ↑
                                              所有权重加起来 = 1.0
```

模型想同时重点关注 "操"（谁在追赶）和 "关公"（追赶谁），但它们**共享同一个概率分布，互相挤压**。给 "关公" 更多权重，"操" 的权重就必须降低。这就像一场会议只能投票选一个议题——不管会议室坐了 64 个人还是 640 个人，投票结果都只有一个。

**维度大小决定的是这组权重算得多精确（会议讨论的质量），但改变不了"只有一个投票结果"这个结构性限制。**

**解决方案：开多场平行会议！** 每场会议独立投票，讨论不同的议题，最后把各场会议的结论合并。这就是**多头注意力**。

```
4 个头 = 4 次独立的 softmax = 4 组互不干扰的注意力分布：

                    "追赶" 往回看时，4 个头各自的注意力分布：

头1（词性搭配）：曹:0.02  操:0.05  引:0.10  兵:0.03  关:0.50  公:0.30
                → "追赶"是动词，关注另一个名词"关公"作为宾语

头2（动作发起者）：曹:0.15  操:0.65  引:0.10  兵:0.05  关:0.03  公:0.02
                  → 关注"操"——谁发起了追赶

头3（动作链）：曹:0.05  操:0.05  引:0.45  兵:0.35  关:0.05  公:0.05
              → 关注"引兵"——追赶之前的动作是什么

头4（相邻搭配）：曹:0.02  操:0.02  引:0.03  兵:0.03  关:0.40  公:0.50
               → 关注紧邻的"关公"——追赶和关公经常一起出现
```

关键点：**4 个头都是在做同一层级的分析（词与词之间的直接关系），只是各自关注不同角度。** 它们面对的是同一段原始文本，同时并行工作，互不干扰。

一句话总结：**维度决定每个注意力分布算得多精确，头数决定能同时维持多少个独立的关注角度。**
...
2. 每个头**独立**做一次完整的自注意力（各自有 Q/K/V 矩阵，各自有独立的 softmax）
3. 把所有头的输出**拼接**起来
4. 通过一个**投影层**合并回原来的维度

```
输入 x (64维)
    ├── 头1: Q1/K1/V1 → softmax₁ → 输出 (16维)   关注"谁是主语"
    ├── 头2: Q2/K2/V2 → softmax₂ → 输出 (16维)   关注"动宾关系"
    ├── 头3: Q3/K3/V3 → softmax₃ → 输出 (16维)   关注"事件链"
    └── 头4: Q4/K4/V4 → softmax₄ → 输出 (16维)   关注"远距离关联"
         │
         └── 拼接 [16+16+16+16 = 64维] → 投影层 → 输出 (64维)
```

总维度没变（还是 64 维），但从 1 次 softmax 变成了 4 次独立的 softmax。每个头的"思考空间"变小了（64 → 16 维），但多个头合在一起覆盖了更多角度。就像四个专家组成的委员会：总人数一样，但四场平行会议能同时覆盖的议题比一场大会多得多。

### 拼接：把会议记录摆在一起

每个头处理完后，会输出一个 16 维的向量。4 个头就有 4 个 16 维向量。**拼接（concatenate）就是把这些向量首尾相连，拼成一个长向量**：

```
以 "操" 这个字为例，4 个头各自输出 16 维：

头1 输出: [0.3, -0.1, ..., 0.5]    ← 16 个数，编码了"谁在行动"的信息
头2 输出: [0.7, 0.2, ..., -0.3]    ← 16 个数，编码了"动作对象"的信息
头3 输出: [-0.1, 0.4, ..., 0.2]    ← 16 个数，编码了"动宾搭配"的信息
头4 输出: [0.5, 0.0, ..., 0.8]     ← 16 个数，编码了"远距离主语"的信息

拼接 → [0.3, -0.1, ..., 0.5, 0.7, 0.2, ..., -0.3, -0.1, 0.4, ..., 0.2, 0.5, 0.0, ..., 0.8]
         ├── 头1的16维 ──┤├── 头2的16维 ──┤├── 头3的16维 ──┤├── 头4的16维 ──┤
                                    共 64 维
```

拼接在代码里就是一行：`torch.cat([h(x) for h in self.heads], dim=-1)`。`dim=-1` 表示在最后一个维度（特征维度）上拼接，所以 4 个 `(B, T, 16)` 变成 `(B, T, 64)`。

到这一步，拼接只是把 4 份会议记录**物理上摆在一起**，彼此之间还没有交互——头 1 的信息不知道头 2 发现了什么。

### 投影层：让各个头的结论互相融合

拼接后的 64 维向量里，前 16 维是头 1 的结论，中间是头 2、3 的，最后 16 维是头 4 的。这些信息还是**各管各的**。

**投影层就是一个普通的线性变换（矩阵乘法）**：一个 64×64 的权重矩阵 W_proj，把拼接后的 64 维映射到新的 64 维：

```
投影层：output = concat × W_proj

         [64维拼接向量]  ×  [64×64 权重矩阵]  =  [64维输出向量]
```

这个矩阵乘法的关键作用是：**输出的每一维都是输入所有 64 维的加权组合**。也就是说，头 1 发现的"谁在行动"可以和头 2 发现的"动作对象"混合在一起，产生更高级的理解——比如"曹操在追赶关公"这样的完整语义。

打个比方：拼接就像四个专家各自提交了一份报告，投影层就像一个总编辑，通读所有报告后写出一份**融合了各方观点的综合结论**。总编辑（W_proj）的权重是训练出来的，模型会自动学会哪些头的意见在什么场景下更重要。
...
先说什么是**线性变换**：就是矩阵乘法，本质上只能做"按比例缩放"和"旋转混合"。不管你叠多少层线性变换，数学上都等价于一层（两个矩阵相乘还是一个矩阵）。通俗的说，矩阵变换不能把直的变成弯的。

线性变换只能表达简单的关系——"如果 A 大，B 就大"。但语言里到处是**非线性的关系**：
...
"大怒"和"大喜"的情感完全相反，但后面接的字却类似；"大喜"和"大军"只差一个字，后面接的字却完全不同。这种**"差一点就完全不同"**的模式，线性变换表达不了——它只能做平滑的、成比例的映射。

**非线性激活函数（ReLU）就像一个开关**：

```
ReLU(x) = x  如果 x > 0  （正数原样通过）
ReLU(x) = 0  如果 x ≤ 0  （负数直接归零）
```

这个简单的开关效果极其强大。经过 "线性层 → ReLU → 线性层" 之后，网络可以：
- **选择性激活**：只让某些特征通过，其余置零——相当于"注意到了这个模式"
- **组合判断**：第一层展开出 256 个"探测器"，每个探测器检测一种模式（比如"前面是大+情感词"、"前面是大+量词"），ReLU 决定哪些探测器被激活，第二层把激活的探测器结果综合成最终判断

类比：注意力是"开会收集各方意见"（线性的信息汇总），FFN 是"回去独立做判断"（非线性的模式识别）。没有 FFN，模型就像一个只会记笔记但不会思考的人——能收集信息，但无法从中提炼出"差一点就完全不同"这类复杂规律。
...
在叠加注意力和 FFN 之后，我们面临一个实际问题：**训练不稳定**。这个问题值得展开说说，因为它是深度学习里最重要的实践挑战之一。

### 什么叫"训练不稳定"？

回忆训练的过程：模型猜下一个字，猜错了就计算 loss（猜得多离谱），然后**反向传播**——从 loss 出发，往回算每个参数该怎么调整才能猜得更准。这个"该怎么调整"的信息就叫**梯度**。

"训练稳定"意味着：每一步训练后，loss 在逐渐下降，模型在持续变好。"训练不稳定"则是：loss 剧烈波动、突然飙升、或者完全卡住不动——模型学不到东西。

### 为什么多层变换会出问题？

当数据经过多层变换时，每一层都会对数值做矩阵乘法。问题是，**矩阵乘法有放大或缩小的效果**：
...
第 1 层后：1.0 × 1.5 = 1.5
第 2 层后：1.5 × 1.5 = 2.25
第 3 层后：2.25 × 1.5 = 3.375
...
第 10 层后：1.0 × 1.5¹⁰ ≈ 57.7        ← 数值爆炸！
第 50 层后：1.0 × 1.5⁵⁰ ≈ 637,621,500  ← 完全失控
```

反过来，如果每层平均缩小到 0.7 倍：

```
第 10 层后：1.0 × 0.7¹⁰ ≈ 0.028    ← 数值消失！
第 50 层后：1.0 × 0.7⁵⁰ ≈ 0.000002  ← 约等于零
```

这就是**数值爆炸**和**数值消失**——向量里的数字要么大到溢出（变成 NaN），要么小到约等于零（丢失所有信息）。层数越多，这个问题越严重。

### 梯度是什么？用爬山来理解

要理解后面的问题，先用一个类比把**梯度**说清楚。

训练模型就像**蒙着眼睛在山里找最低谷**（loss 最小的地方）。你看不见地形，唯一的感知方式是**用脚试探脚下的坡度**——往哪个方向倾斜、有多陡。这个坡度就是**梯度**。

- 梯度大 = 脚下很陡，说明离谷底还远，得大步走
- 梯度小 = 脚下很平，说明快到谷底了（或者遇到平地了），小步走
- 梯度的方向 = 告诉你该往哪个方向迈步

每次训练，模型就是"试探坡度 → 迈一步 → 再试探 → 再迈一步"，一步步走向谷底。

### 梯度传不回去 = 脚下失去感知

在多层网络中，训练时梯度要从最后一层**逐层往回传**到第一层（反向传播）。每个层的参数都需要收到梯度信号才知道该怎么调整自己。

问题来了：梯度在逐层传递的过程中，同样面临放大/缩小的问题。
...
脚下完全感觉不到坡度，不知道该往哪走，也不知道该走多远。模型的前面几层收到的梯度接近零，参数纹丝不动——这就是**"模型学不动"**。
...
脚下突然极其陡峭，算出来的梯度大到离谱，参数一下子更新了一个巨大的幅度——直接从悬崖摔下去了，loss 飙到天上去。模型直接"崩溃"（loss 变成 NaN）。

### 残差连接（Residual Connection）：给你一根登山绳
...
技术上，这一个 "+" 号解决了梯度消失的问题。回到爬山的比喻：

没有残差连接时，你只有一条路下山——必须穿过每一层的"地形"。如果中间某层是一片大平地（梯度消失），你就卡死了。

```
没有残差连接：
  你 → [穿过第3层的平地] → [穿过第2层] → [穿过第1层]
        ↑ 梯度在这里消失了，后面的层完全收不到信号
```

有了残差连接，相当于**在每一层都架了一根登山绳直通山脚**。不管中间的地形多复杂——平顶山、悬崖、沼泽——你都可以顺着绳子把坡度信号完整地传回去：

```
有残差连接：
  你 → [穿过第3层] → [穿过第2层] → [穿过第1层]   （可能信号很弱）
  你 → [顺着绳子直通] → 第2层 → 第1层             （信号完整保留）
```

"+" 号这条路的梯度**永远是 1**（加法的导数是 1），就像一条没有任何起伏的绳索——不管网络有多深，梯度都能沿着它完整地传回最前面的层。这是深层网络能训练起来的关键。

### LayerNorm（层归一化）

经过注意力层和残差连接后，向量里各个维度的数值范围可能**差别很大**。什么叫差别大？看个例子：

```
"操" 这个字经过注意力层后的 64 维向量（截取前 8 维）：

  [120.5, -0.003, 45.2, 0.0001, -88.7, 0.52, 203.1, -0.007]
   ↑ 很大          ↑ 接近零          ↑ 很大         ↑ 接近零
```

有的维度数值高达 200 多，有的维度只有 0.0001。这种情况下，后续的矩阵乘法和 ReLU 会出什么问题？

**大数值主导一切**：矩阵乘法是加权求和，数值大的维度天然权重高。那些 0.0001 的维度虽然可能携带了重要信息，但在计算中完全被淹没了——就像一场讨论会上有人用大喇叭说话，其他人的发言根本听不见。

**学习率顾此失彼**：训练时所有维度共享同一个学习率。对数值 200 的维度来说，学习率 0.001 意味着每步调整 0.2，尺度合理；但对数值 0.0001 的维度来说，同样的学习率意味着每步调整 0.0000001，几乎没在动。**模型没法同时照顾好所有维度**。

LayerNorm 的做法很简单：**把每个向量"拉齐"到均值 0、标准差 1 的范围**。

```
归一化前：[120.5, -0.003, 45.2, 0.0001, -88.7, 0.52, 203.1, -0.007]
归一化后：[0.85,  -0.12,  0.31, -0.11,  -0.67,  0.02,  1.52, -0.13]
                    ↑ 所有维度都在差不多的范围内了
```
...
归一化后，每个维度的数值都在 -2 到 +2 左右的范围内，大家站在同一起跑线上。后续的矩阵乘法能公平地利用每一个维度的信息，学习率也能均匀地作用于所有维度。
