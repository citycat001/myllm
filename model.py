    ...
    def forward(self, idx: torch.Tensor, targets: torch.Tensor | None = None):
        ...
        参数：
            idx:     输入的字（已转成数字），形状 (B, T)
                     B = 一批处理多少条，T = 每条多少个字
            targets: 正确答案（训练时用），推理时不传

        返回：
            logits: 模型对下一个字的"打分"，形状 (B, T, C)
                    C = 词表大小，即有多少个候选字
            loss:   模型猜错了多少（训练时用来改进模型）
        """
        ...
        if targets is not None:
            ...
            #
            # 打个比方：模型说"曹后面有 60% 概率是操，30% 是军"，
            # 而正确答案是"操"，那 loss 就比较小（猜对了概率高）。
            # 如果正确答案是"营"但模型只给了 1% 的概率，loss 就很大（猜得很差）。
            #
